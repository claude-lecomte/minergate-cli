...
inline h256 arrayToH256(const QByteArray &ar) {
  Q_ASSERT(ar.size() == 32);
  return h256(reinterpret_cast<const byte *>(ar.constData()), h256::ConstructFromPointer);
}

inline QByteArray h256ToArray(const h256 &h) {
  QByteArray ret(reinterpret_cast<const char*>(&h), 32);
  Q_ASSERT(ret.size() == 32);
  return ret;
}
