...
public:
  ...
  {
    FullAllocation(ethash_light_t _light, ethash_callback_t _cb, bool _dagInMemory);
    ~FullAllocation();
    EthashProofOfWork::Result compute(h256 const& _headerHash, Nonce const& _nonce) const;
    bytesConstRef data() const;
    uint64_t size() const {
  ...
  static uint64_t cacheSize(uint64_t _number);
  ...
  static unsigned computeFull(h256 const& _seedHash, bool dagInMemory, bool _createIfMissing = true);
  ...
  static FullType full(h256 const& _seedHash, bool _dagInMemory, bool _createIfMissing = false, std::function<int(unsigned)> const& _f = std::function<int(unsigned)>());
  ...
  void setDAGDirName(const char *custom_dir_name);
private:
  EthashAux() {}
